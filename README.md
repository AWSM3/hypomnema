...
изменился, прежняя проверка больше не считается актуальной. Когда агент запускает
проверку через Hypomnema, система сама фиксирует её завершение, отпечаток полного
вывода и диагностический фрагмент. Ошибка, превышение срока или изменение
проверяемого результата не могут превратиться в успешную проверку.

Для важных выводов сбор оснований и оценка разделены: сначала готовится небольшая
неизменяемая выборка, затем она проверяется отдельно. Агент ограничивает время
ожидания такой проверки. Если она не завершилась или оснований недостаточно,
работа продолжается с явной отметкой об отсутствии независимого подтверждения.
